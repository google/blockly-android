...
public class BlockTest {
    ...
    public void testMessageTokenizer() {
        ...
        assertWithMessage("Should have 1 token: " + tokens.toString()).that(tokens.size())
            .isEqualTo(1);
    ...
    public void testSerializeBlock() throws BlocklySerializerException, IOException {
        BlockFactory bf = new BlockFactory(InstrumentationRegistry.getContext(),
            new int[]{R.raw.test_blocks});
    ...
    private void assertListsMatch(List<String> expected, List<String> actual) {
        assertWithMessage("Wrong number of items in the list.").that(actual.size())
            .isEqualTo(expected.size());
        for (int i = 0; i < expected.size(); i++) {
            assertWithMessage("Item " + i + " does not match.").that(actual.get(i))
    ...
    public void testEscapeFieldData() {
        ...
        assertThat(matcher.group()).isEqualTo("less than &lt; greater than &gt; " +
            "ampersand &amp; quote \" apostrophe ' end");
    }
