...
public abstract class Field extends Observable<Field.Observer> implements Cloneable {
    ...
    public static final int TYPE_MUTATOR = 10;

    @Retention(RetentionPolicy.SOURCE)
    @IntDef({TYPE_UNKNOWN, TYPE_LABEL, TYPE_INPUT, TYPE_ANGLE, TYPE_CHECKBOX, TYPE_COLOR,
            TYPE_DATE, TYPE_VARIABLE, TYPE_DROPDOWN, TYPE_IMAGE, TYPE_NUMBER, TYPE_MUTATOR})
    ...
    // Mutator fields are not explicitly declared, so they have no string representation
    ...
    public void serialize(XmlSerializer serializer) throws IOException {
        if (mType == TYPE_LABEL || mType == TYPE_IMAGE || mType == TYPE_MUTATOR) {
            return;
        }
